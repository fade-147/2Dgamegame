...
public class Boss2Rabbit22 : MonoBehaviour
{
    [Header("基础配置")]
    ...
    public float dashSpeed = 35f; // 冲刺速度（越大越快）
    ...
    void TriggerAttack(BossAttackType attackType)
    {
        ...
        {
            StartCoroutine(fiveTwoPaodan());
            
        }
    }

    IEnumerator fiveTwoPaodan()
    {
        SpawnThreeBullets();
    ...
    private IEnumerator TeleportAttackCoroutine()
    {
        ...
        // 平滑缩小Boss至消失
        yield return StartCoroutine(ScaleBoss(Vector3.zero, scaleAnimDuration));

        //  放慢全局时间流速
        float originalFixedDeltaTime = Time.fixedDeltaTime; // 保存原始物理时间
        Time.timeScale = slowTimeScale;
        Time.fixedDeltaTime = originalFixedDeltaTime * slowTimeScale; // 同步物理时间

        //  计算玩家身后的目标位置
        Vector3 playerForwardDir = -playerTransform.right; // 2D默认right为朝向，取反=身后
        // 如果玩家有左右翻转（比如localScale.x正负），修正朝向
        TurnToPlayer();
        // 计算最终瞬移位置
        Vector3 targetPos = playerTransform.position + playerForwardDir.normalized * teleportDistance;
        // 确保Boss和玩家在同一Y轴高度
        targetPos.y = transform.position.y+4f;

        //  瞬移到目标位置
        transform.position = targetPos;

        // 平滑放大Boss至原始尺寸
        yield return StartCoroutine(ScaleBoss(new Vector3 (1,1,1), scaleAnimDuration));
    ...
    private IEnumerator ScaleBoss(Vector3 targetScale, float duration)
    {
        ...
        while (elapsedTime < duration)
        {
            ...
            t = Mathf.SmoothStep(0, 1, t); // 平滑插值，让缩放更自然
    ...
    public void SpawnThreeBullets()
    {
        ...
        {
            new Vector2(baseSpawnPos.x - 3f, baseSpawnPos.y),
            new Vector2(baseSpawnPos.x - 5f, baseSpawnPos.y), // 左子弹
            new Vector2(baseSpawnPos.x, baseSpawnPos.y),       // 中子弹
            new Vector2(baseSpawnPos.x + 3f, baseSpawnPos.y),
            new Vector2(baseSpawnPos.x + 5f, baseSpawnPos.y)  // 右子弹
        };
